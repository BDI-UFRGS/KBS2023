...
class MultiClassDatasetReader:
    def __init__(self, file_name: str) -> None:
        ...
        with open(file_name, newline='') as f:
            ...
            for row in spamreader:
                entity = dict()
                entity['class'] = re.sub('[^A-Za-z]+', ' ', row[0]).strip()
                entity['term'] = row[1]
                entity['comment'] = row[2]
                entity['term+comment'] = f'{row[1]} is defined as {row[2]}'
    ...
    def keep_only_n_classes(self, n: int) -> None:
        value_counts = self.df[self.df.columns[0]].value_counts()
        elegible_indexes = value_counts.index[0:n]
        new_df = pd.DataFrame()
        for index in elegible_indexes:
            item = self.df[self.df[self.df.columns[0]] == index]
    ...
    def keep_only_classes_with_more_than_n_instances(self, n: int) -> None:
        ...
        for i in range(len(value_counts)):
            if value_counts[i] >= n:
                item = self.df[self.df[self.df.columns[0]] == value_counts.index[i]]
                new_df = pd.concat([new_df, item])
    ...
    def value_counts(self) -> None:
        return self.df[self.df.columns[0]].value_counts()


    def get_minority_class_size(self) -> int:
        return math.ceil(min([self.df[self.df[self.df.columns[0]] == index].shape[0] for index in self.value_counts().index]))
    ...
    def count_graph(self) -> None:
        ...
        sns.countplot(x = self.df[self.df.columns[0]])
        plt.tight_layout()
        plt.show()